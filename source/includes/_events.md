...
| customer_bill_payment_created | Represents a `pending` Regalii transaction status. This is only part of the bill payment integration. |
| customer_bill_payment_completed | Represents a `confirmed` Regalii transaction status and a `processed` Dwolla transfer status. |
| customer_bill_payment_failed | Represents a `rejected` Regalii transaction status and a `failed` Dwolla transfer status. |
